...
loadTemplate(contextPath+"/resources/corner/templates/viewport.html", function(element) {
    viewportTemplate = element;
});

var studyViewerTemplate; // the study viewer template
loadTemplate(contextPath+"/resources/corner/templates/studyViewer.html", function(element) {
...
if(studyIdIni==="NA"){
	...
	$.getJSON(contextPath+'/apidicom/dicomne/'+pacienteId1, function(data) {
	  data.studyList.forEach(function(study) {
	    ...
	    $(studyRowElement).click(function() {
	      ...
	      console.log("ESTUDIO JSON");
	      console.log(study.studyId + ".json<");
		...
		// Get study list from JSON manifest
	$.getJSON(contextPath+'/apidicom/dicomne/'+pacienteId1+"/"+studyIdIni, function(data) {
	  data.studyList.forEach(function(study) {
	  ...
	  //Cargar el inicial
	     // Add new tab for this study and switch to it
	      var studyTab = '<li><a href="#x' + pacienteId1 + '" data-toggle="tab">' + pacienteId1 + '</a></li>';
	      $('#tabs').append(studyTab);
	      ...
	      studyViewerCopy.attr("id", 'x' + pacienteId1);
	      // Make the viewer visible
	      studyViewerCopy.removeClass('hidden');
	      // Add section to the tab content
	      studyViewerCopy.appendTo('#tabContent');
	    ...
	    //  console.log("ESTUDIO JSON2");
	     // console.log(study.studyId + ".json<");
	      loadStudy(studyViewerCopy, viewportTemplate, studyIdIni + ".json");  
	  //FIN CARGA INICIAL
