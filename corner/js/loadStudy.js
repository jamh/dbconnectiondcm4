...
function loadStudy(studyViewer, viewportModel, studyId) {
  ...
  $.getJSON(contextPath+'/apidicom/dicomnv/'+pacienteId1+'/'+studyId.replace(".json",""), function(data) {
            ...
            if (series.numberOfFrames !== undefined) {
                ...
                for (var i = 0; i < numberOfFrames; i++) {
                    ...
                    if (imageId.substr(0, 4) !== 'http') {
                      ...
                      //  imageId = "dicomweb://localhost:8090/dicom/" + imageId;
                     // imageId = "dicomweb://localhost:3000/resources" + image.imageId;
                      imageId = urlResponseImg + image.imageId;
                    }
                ...
                series.instanceList.forEach(function(image) {
                    ...
                    //   imageId = "dicomweb://localhost:8090/dicom/" + image.imageId;
                     imageId = urlResponseImg + image.imageId;
                    }
                    stack.imageIds.push(imageId);
                });
       ...
       var  iPDfFirst = 0;
        ...
        imageViewer.stacks.forEach(function(stack, stackIndex) {
 			...
 			//Agrega el primer link a pdf
            if(iPDfFirst==0 && flagPDF==='1'){
			      seriesEntry2 = '<a class="list-group-item" ' +
			     'oncontextmenu="return false" ' +
			     'unselectable="on" target="_blank"  href="'+contextPath+'/apidicom/pdf/'+pacienteId1+'/'+studyId.replace(".json","")+'"' +
			     ...
			     'unselectable="on"' +
			     'onselectstart="return false;"' +
			     'onmousedown="return false;"> <div class="container d-flex justify-content-center align-items-center"> <img src="'+contextPath+'/resources/corner/img/pdf6.png" alt="Interpretacion"></div></div>' +
			     "<div class='text-center small'>" + "PDF Interpreatacion" + '</div></a>';
			...
			if(iPDfFirst==0 && flagPDF==='1'){
				 // Add to series list
	            var seriesElement = $(seriesEntry2).appendTo(seriesList);
	               seriesElement = $(seriesEntry).appendTo(seriesList);
	               iPDfFirst++;
			}else{
